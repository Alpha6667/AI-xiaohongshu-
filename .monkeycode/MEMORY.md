...
[后台登录与小红书登录合一偏好]
- Date: 2026-04-24
- Context: 用户明确希望前端产品形态尽量做到“登录一次即可进入后台并直接使用小红书能力”，避免额外账号绑定步骤
- Instructions:
  - 产品体验应优先朝“用小红书账号登录后台”或“首次登录即自动完成小红书连接”的一体化方向设计。
  - 默认不优先引入独立的后台账号体系加手动绑定小红书账号的双账户流程，除非真实技术限制无法避免。
  - 后续设计需区分“用户感知的一次登录”与“系统内部会话/授权存储”的实现差异，但对外流程应尽量简化为单账号入口。

[OpenClaw 与网页双端一致产品链路]
- Date: 2026-04-24
- Context: 用户明确说明希望 OpenClaw、QQ 和 HTML 后台围绕同一份内容对象协同工作
- Instructions:
  - 用户先通过 OpenClaw 对话下达主题需求，例如“出一个关于乌鸦喝水的故事”。
  - OpenClaw 将消息转发给后台，后台生成文案和图片，并把同一份结果同时提供给 OpenClaw 和网页端展示。
...
[小红书新版图文发布真实流程]
- Date: 2026-04-29
- Context: Agent 在完成一次真实小红书发布成功后记录
- Category: 代码模式
- Instructions:
  - 当前小红书新版发布页下，不能假设进入发布页后就能直接填标题正文；实际应先上传图片，页面才会切到图文编辑模式。
  - 标题输入框的稳定定位特征是 `placeholder="填写标题会有更多赞哦"`，正文区域使用 `contenteditable`。
  - 发布前应先核对目标 `postId` 是否已是 `published`，避免重复发布已成功的内容；本次真实成功发布的对象是 `post_e4c2534c7a`，而 `post_877f43cbed` 在更早之前已成功发布，不应重复发送。
  - 登录态是否有效，应以真实提交动作是否被 `redirectReason=401` 打回登录页为准，而不应仅以“能打开发布页”判断。

[小红书项目当前收尾优先级]
- Date: 2026-04-29
- Context: Agent 在整理项目最后收尾事项清单时记录
- Category: 代码模式
- Instructions:
  - 当前最高优先级不是继续扩新功能，而是固化真实发布脚本、补发布前防重复检查、完成发布后数据回收验证。
  - 后续所有真实发布前，都应先检查目标 `postId` 是否已是 `published`，避免重复发送已成功内容。
  - 当前阶段应优先沉淀“稳定复用”和“发布后追踪”能力，而不是继续扩模型排障或大范围重构。

[模型接入先出任务单，不阻塞于 Key]
- Date: 2026-04-29
- Context: 用户要求先把生图模型厂商切换、Key 配置和默认模型切换能力拆成前后端任务，再交给前后端执行
- Instructions:
  - 这类需求优先先写成前后端协作文档，由前后端按文档直接开发，我负责拆解、协调和推送文档。
  - 当前即使还没有真实 API Key，也不应阻塞前后端先完成配置页、后端配置接口、厂商切换和默认模型映射等非密钥依赖工作。
  - API Key 不应下发到浏览器明文使用；前端只负责录入和脱敏展示，真实第三方模型调用必须由后端发起。

[服务器联调优先走最小单机部署]
- Date: 2026-04-29
- Context: 用户要求把整套项目放到自己的服务器上做测试，并希望我提供可直接交给 OpenClaw 执行的最小上线方案
- Instructions:
  - 当前服务器部署优先采用“前端 + 后端 + Nginx”的单机最小上线方案，不先追求完整生产化。
  - 当前后端主持久化依赖 `backend/data/repository.json`，可以先不接 PostgreSQL 和 Redis 也完成这轮最小部署。
  - 部署目标优先是让用户能通过自己的域名打开前端页面并继续联调，再根据服务器上的真实问题补剩余功能。
...
[帖子 metrics 刷新与展示偏好]
- Date: 2026-05-13
- Context: 用户指出后台 60 秒自动刷新过快，并要求帖子数据样式参考截图的数据卡片风格
- Instructions:
  - 帖子 metrics 自动刷新间隔应为 3 小时，保留手动刷新能力。
  - 帖子详情数据展示应采用两行三列统计卡样式，每项包含指标名、蓝色大号数值和“昨日 --”辅助信息。
  - 数据卡片视觉应接近截图：浅蓝灰背景、细边框、充足留白、当前重点卡片可使用更明显浅蓝选中态。
